...
public class QiniuUtils {
    ...
    public static String upload2Qiniu(byte[] bytes, String fileName) {
        ...
        String upToken = auth.uploadToken(DevOpsConstant.BUCKET);
        try {
            Response response = uploadManager.put(bytes, "img/"+ fileName, upToken);
            //解析上传成功的结果
            DefaultPutRet putRet = new Gson().fromJson(response.bodyString(), DefaultPutRet.class);
            //返回文件完整路径
     ...
     * @return String
     */
    public static String uploadHeadQiniu(){
        byte[] bytes = null;
        try {
            bytes = ImgUtil.getImg(DevOpsConstant.IMG_RANDOM_D);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        String format = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
        //进行拼接 时间+UUID+原始文件名+文件后缀名
        String fileName = format + "-" + UUID.randomUUID().toString().substring(0, 6);
        //使用工具类将文件上传到七牛云服务器 返回上传图片链接
        return QiniuUtils.upload2Qiniu(bytes, fileName);
    }
}
