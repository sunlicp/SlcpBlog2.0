...
public class TagServiceImpl extends ServiceImpl<ITagMapper, Tag> implements ITagService {
    ...
    public List<TagDTO> getAllTag() {
        List<TagDTO> tags = this.baseMapper.listTags();
        for (TagDTO tag : tags) {
            tag.setColor(StrUtil.format("background: linear-gradient(to bottom right, {} 0, {} 100%)", ColorUtil.getRandColor(), ColorUtil.getRandColor()));
        }
        return tags;
    }
    ...
    @Override
    @DoQueryCache(expireTime = 86400)
    public String getStringByTag() {
        return DevOpsConstant.TAG_STR + StringUtil.getTagStr(this.getAllTag());
    }
