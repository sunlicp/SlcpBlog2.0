...
public class UserController {
    ...
    @Operation(summary = "map", description = "获取位置")
    public void map(@PathVariable("id") Long id) {
        //获取当前地理位置
        JSONObject ipJson = ImgUtil.getIp();
        adminService.lambdaUpdate().set(SysAdmin::getLastLoginAddress, ipJson.getString("regionName") + ipJson.getString("city")).set(SysAdmin::getLastLoginTime, LocalDateTime.now()).eq(SysAdmin::getId, id).update();
    }

    @PostMapping("/users")
    @Operation(summary = "saveOrUpdateOfUser", description = "用户保存修改")
    ...
    @Operation(summary = "updRoleIdOfUser", description = "根据用户id修改角色")
