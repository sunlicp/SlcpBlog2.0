...
public class ArticleController {
    ...
    public String article(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum, Model model) {
        ...
        model.addAttribute("tagStr", tagService.getStringByTag());
        return "article";
    }
    ...
    public String[] getPoem() {
        ...
        if (200 == resp.getStatus()) {
            ...
            poem = json.getString("content");
        }
        httpRequest = HttpUtil.createGet(DevOpsConstant.FAMOUS_SAYING);
        resp = httpRequest.execute();
        if (200 == resp.getStatus()) {
            String data = resp.body();
            JSONObject obj = JSONObject.parseObject(data);
            famousSaying = obj.getString("hitokoto") + "     —" + obj.getString("source");
        }
        return new String[]{poem, famousSaying};
    }
}
